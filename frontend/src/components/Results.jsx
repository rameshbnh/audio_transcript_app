export default function Results({ result, mode }) {
  ...
  if (mode === "transcribe") {
    ...
    if (result.segments && Array.isArray(result.segments)) {
      displayText = result.segments.map(segment => `[${segment.start}-${segment.end}] ${segment.text}`).join('\n');
    } else {
    ...
    title = "Diarization";
    if (result.segments && Array.isArray(result.segments)) {
      displayText = result.segments.map(segment => `${segment.speaker}: ${segment.text}`).join('\n');
  ...
  const downloadMarkdown = () => {
    ...
    if (mode === "transcribe") {
      if (result.segments && Array.isArray(result.segments)) {
        content = result.segments.map(segment => `[${segment.start}-${segment.end}] ${segment.text}`).join('\n');
      } else {
        content = result.transcript || JSON.stringify(result, null, 2);
      }
    } else if (mode === "diarize") {
      if (result.segments && Array.isArray(result.segments)) {
        content = result.segments.map(segment => `${segment.speaker}: ${segment.text}`).join('\n');
      } else if (result.transcript) {
